...
export async function POST(req: Request) {
   ...
   if (evt.type === "user.updated") {
      console.log("user updated event triggered -> ", evt.data);
      const { id, username, image_url, email_addresses } = evt.data as UserJSON;
      ...
      try {
         ...
         if (!res.ok) {
            console.error(
               "[webhook] Backend rejected user update:",
               res.status,
               responseText,
            );
            return new Response(`Backend error: ${responseText}`, {
   ...
   if (evt.type === "user.deleted") {
      console.log("user deleted event triggered -> ", evt.data);
      const { id } = evt.data;
      ...
      try {
         ...
         if (!res.ok) {
            console.error(
               "[webhook] Backend rejected user deletion:",
               res.status,
               responseText,
            );
            return new Response(`Backend error: ${responseText}`, {
