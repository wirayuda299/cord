...
export async function createRole(
   p: CreateRolePayload,
): Promise<APIResponse<CreateRoleApiResponse>> {
   ...
   try {
      ...
      const json: APIResponse<CreateRoleApiResponse> = await res
         .json()
         .catch(() => null);
...
export async function updateRole(p: UpdateRolePayload): Promise<APIResponse> {
   const { getToken } = await auth.protect();
   try {
      const token = await getToken();
