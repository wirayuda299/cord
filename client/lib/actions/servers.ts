...
export async function joinServer(
   ...
   try {
      ...
      if (!res.ok) {
         return {
            message: json.message ?? "failed to join server",
            success: false,
         };
      }
...
export async function updateServer({
   ...
   payload,
   fields,
}: UpdateServerProps): Promise<APIResponse> {
...
export async function updateSafetySetup(
   serverId: string,
   data: {
   ...
   try {
      ...
      if (!res.ok) {
         return {
            message: json.message ?? "failed to update safety rules",
            success: false,
         };
      }
...
export async function banMember(
   ...
   try {
      ...
      if (!res.ok) {
         return {
            message: json?.message ?? "failed to ban member",
            success: false,
         };
      }
      ...
      return { message: "failed to ban member", success: false };
   }
}
...
export async function kickMember(
   ...
   try {
      ...
      if (!res.ok) {
         return {
            message: json?.message ?? "failed to kick member",
            success: false,
         };
      }
...
export async function unbanMember(
   ...
   try {
      ...
      const res = await fetch(
         `${getPublicApiUrl()}/server/bans?serverID=${serverId}&memberID=${memberId}`,
         {
            method: "DELETE",
            headers: {
               Accept: "application/json",
      ...
      if (!res.ok) {
         return {
            message: json.message ?? "failed to unban member",
            success: false,
         };
      }
...
export async function deleteServer(serverId: string): Promise<APIResponse> {
   ...
   try {
      ...
      const res = await fetch(
         `${getPublicApiUrl()}/server?serverID=${serverId}`,
         {
            method: "DELETE",
            headers: {
               Accept: "application/json",
      ...
      if (!res.ok) {
         return {
            message: json.message ?? "failed to delete server",
            success: false,
         };
      }
