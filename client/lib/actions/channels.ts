...
export async function createChannel(
   ...
   if (!parsed.success) {
      return {
         message: parsed.error.message ?? "invalid data",
         success: false,
      };
   }
   try {
      ...
      return {
         message: result.message ?? "failed to create channel",
         success: false,
      };
   } catch (e) {
      return {
...
export async function updateChannel(
   ...
   const parsed = createChannelSchema
      .pick({ name: true, topic: true })
      .safeParse({
         name: data.name,
         topic: data.topic,
      });
