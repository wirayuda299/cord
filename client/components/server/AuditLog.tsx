...
function mapActionTypeToCategory(type: string): ActionCategory {
   ...
   if (type.startsWith("server_") || type.startsWith("safety_setup_"))
      return "server";
...
function LogSkeleton() {
   return (
      <div
         className="flex flex-col gap-2 px-1 py-2 animate-pulse"
         aria-hidden="true"
      >
...
export default function AuditLog({ serverId }: { serverId: string }) {
   ...
   useEffect(() => {
      ...
      async function loadLogs() {
         setStatus("loading");
         try {
            const { success, data } = await getAuditLogs(serverId);
            if (cancelled) return;
            ...
            {status === "loading" ? (
               ...
               <EmptyState
                  ...
                  icon={
                     <AlertTriangle
                        size={28}
                        className="opacity-50 text-red-400"
                     />
                  }
