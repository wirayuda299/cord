...
import { Hash, ChevronRight, MessageSquareText, ArrowLeft } from "lucide-react";
...
export default function ChannelHeader({ channel, actions }: ChannelHeaderProps) {
  const params = useParams<{ id: string; channel_id: string; thread_id?: string }>();
  const thread_id = params?.thread_id;
  const isInThread = Boolean(thread_id);

  const { data: threadData, error, isLoading } = useSWR(
    thread_id ?? null,
    getThreadById,
  );
  ...
  const backHref = isInThread ? `/${params.id}/${params.channel_id}` : undefined;
    ...
    <phantom-ui loading={isLoading}>
      <header className="h-14 shrink-0 px-2.5 shadow gap-4 border-b border-gray-600/50 flex items-center">
        <div className="flex items-center gap-2 w-full min-w-0">
          ...
          <div className={cn("flex items-center gap-1.5 shrink-0", isInThread && "opacity-60")}>
          ...
          {isInThread && (
            <>
              ...
              <div className="flex items-center gap-1.5 bg-indigo-500/10 border border-indigo-500/25 rounded-full px-2.5 py-0.5 min-w-0">
                <MessageSquareText size={13} className="text-indigo-400 shrink-0" />
