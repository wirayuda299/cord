...
import { getSafetySetup, updateSafetySetup } from "@/lib/server/actions/servers";
...
function ToggleRow({
   ...
   return (
      <div className="flex items-start justify-between gap-4 p-4 rounded-xl bg-white/2 border border-white/5">
...
export default function SafetySetup({ serverId }: { serverId: string }) {
   ...
   const onSubmit = async (values: SafetyFormValues) => {
      ...
      try {
         const { error } = await updateSafetySetup(serverId, values);
         if (error) {
            setSubmitStatus({
               type: "error",
               message: error,
