...
function UsageBar({ uses, maxUsers }: { uses: number; maxUsers: number }) {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full sm:min-w-32 sm:w-32">
...
function CopyButton({ code }: { code: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handle}
      className="flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-white/60 hover:text-white text-xs font-medium transition-colors whitespace-nowrap"
    >
...
function InviteRow({
  ...
  return (
    <div className={cn(
      "flex flex-col gap-3 md:grid md:grid-cols-[1fr_9rem_7rem_auto] md:items-center md:gap-4 px-4 py-3.5 rounded-xl border transition-colors",
      full
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-sm font-mono text-white truncate max-w-[14rem] sm:max-w-xs">{code}</p>
      ...
      <div className="flex items-center gap-2 md:justify-self-end">
        ...
        <button
          ...
          className="flex items-center justify-center size-7 rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors shrink-0"
        >
          <Trash2 size={13} />
        </button>
      </div>
...
function CreateInviteForm({ onClose, onCreate }: {
  ...
  return (
    <div className="rounded-xl border border-discord-blue/30 bg-discord-blue/5 p-4 sm:p-5 flex flex-col gap-4">
  ...
  return (
    <phantom-ui loading={isLoading}>
      <div className="flex flex-col w-full max-h-screen overflow-hidden text-white">
        <div className="px-4 sm:px-8 pt-6 sm:pt-8 pb-5 shrink-0 border-b border-white/5">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-discord-blue hover:opacity-90 text-white text-sm font-medium transition-opacity w-full sm:w-auto"
            >
              <Plus size={14} />
              Create Invite
            </button>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-5 flex flex-col gap-3">
          {creating && (
            <CreateInviteForm
