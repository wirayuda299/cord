  ...
  return (
    <div className="rounded-2xl overflow-hidden w-full bg-[#111214] shadow-2xl ring-1 ring-white/10">
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg,#5865f2 0%,#4752c4 50%,#3b3fa8 100%)",
        }}
      />
      ...
      <div className="px-3 py-3 space-y-3">
        {bio ? (
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#f2f3f5] mb-1">
              About Me
            </p>
            <p className="text-xs text-[#b5bac1] leading-relaxed line-clamp-4">
              {bio}
            </p>
          </div>
        ) : (
...
function Section({
  ...
  return (
    <div className="space-y-2">
      <div>
        <p className="text-xs font-bold uppercase tracking-wider text-[#f2f3f5]">
          {label}
        </p>
...
function ProfileEditorContent({ serverId }: Props) {
  const cacheKey = serverId ? `/server/profile/${serverId}` : null;
  const { data, isLoading } = useSWR(cacheKey, () =>
    getServerProfile(serverId),
  );
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto p-3 md:p-10">
          <phantom-ui loading={isLoading}>
            <div className="space-y-8">
              ...
              <Section
                label="Server Avatar"
                hint="Replaces your global avatar in this server."
              >
                <div
                ...
                >
                  ...
                  <div className="space-y-2.5">
                    ...
                    <p className="text-xs text-[#6d6f78]">
                      Recommended: 128×128 px or larger
                    </p>
                    {fileErrors.map((e, i) => (
                      <p key={i} className="text-xs text-[#f23f42]">
                        {e}
                      </p>
                    ))}
                  </div>
                    ...
                    render={() => (
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          addFiles(files);
                          if (files.length > 0)
                            form.setValue("avatar", files[0].name, {
                              shouldDirty: true,
                              shouldTouch: true,
                            });
                        }}
              ...
              <Section
                label="Display Name"
                hint="How your name appears to others in this server."
              >
                <Controller
                  control={form.control}
                  name="username"
                  render={({ field, fieldState }) => (
                    <div className="space-y-1.5">
                      ...
                      {fieldState.error && (
                        <p className="text-xs text-[#f23f42]">
        ...
        <div className="w-70 shrink-0 overflow-y-auto h-full border-l border-white/6 bg-[#2b2d31]/30 px-6 py-10 hidden xl:flex flex-col gap-4">
          ...
          <p className="text-[10px] text-[#4e5058] leading-relaxed">
            This is how your profile card appears to other members in this
            server.
          </p>
        </div>
      </div>
    ...
    <Dialog>
      ...
      >
        <div className="relative h-screen flex flex-col">
          <ProfileEditorContent serverId={serverId} />
        </div>
      </DialogContent>
    </Dialog>
  );
