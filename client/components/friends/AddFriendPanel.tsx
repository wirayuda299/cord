...
export default function AddFriendPanel() {
  ...
  const {
    trigger: triggerSendFriendRequest,
    error: sendError,
  ...
  const handleSendFriendRequest = async (user: Friend) => {
    ...
    try {
      await triggerSendFriendRequest({ userId: user.id })
  ...
  return (
    <div className="block max-w-xl">
      <div className="flex flex-col gap-4">
        ...
        {!isSearching && shouldSearch && users.length === 0 && !searchError && (
          <p className="text-sm text-zinc-500">
            No users found.
          </p>
        )}
        ...
        <div className="flex flex-col gap-1">
          {users.map(user => {
            ...
            else if (user.friend_status === 'pending') buttonLabel = 'Sent'
            else if (user.friend_status === 'accepted') buttonLabel = 'Friends'
            else if (user.friend_status === 'blocked') buttonLabel = 'Blocked'
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="shrink-0 rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-zinc-500"
                >
                  {buttonLabel}
                </button>
              </div>
