...
export default function PendingRequests({ currentUser }: { currentUser: string }) {
  ...
  const incoming =
    pendingRequests?.filter((r) => r.addressee_user_id === currentUser) ?? [];
  const outgoing =
    pendingRequests?.filter((r) => r.requester_user_id === currentUser) ?? [];
            ...
            <ul className="space-y-1">
              {outgoing.map((req) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <Avatar
                      name={req.addressee_username}
                      avatarURL={req.addressee_avatar_url}
                    />
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-zinc-100 truncate">
