...
*   **Go REST & WebSocket API**: High-performance HTTP server handling authentication, database updates, and real-time connections. A pool of queue-worker goroutines runs in this same process (see 3.4) rather than as a separate deployment.
...
> [!NOTE]
> Redis was used earlier in the project's history as the background-job broker, but has since been removed in favor of an in-process Go channel (see 3.4) — there is no Redis dependency in the current codebase.
...
graph TD
    ...
    GoAPI -->|Enqueue Jobs / Go channel| GoWorker[Queue-worker goroutines]
...
sequenceDiagram
    ...
    Frontend->>Backend: Forward POST /users/create (X-Internal-Secret header)
    Backend->>Backend: Compare header against INTERNAL_API_SECRET
...
> [!IMPORTANT]
> `POST /users/create` (the only database sync endpoint currently exposed — see [users_routes.go](backend/internal/routes/users_routes.go); there is no `/users/update` or `/users/delete` route) is unprotected by the Clerk auth middleware, since its caller is Next.js's [webhook route.ts](client/app/api/webhook/route.ts) after Svix verification, not an end-user's browser. Because `NEXT_PUBLIC_API_URL` is a public, client-bundled value, the Go endpoint is directly reachable by anyone, so it additionally requires a shared `X-Internal-Secret` header (checked in [users.go](backend/internal/handlers/users.go) against the `INTERNAL_API_SECRET` env var, set identically on both the client and backend) proving the call actually came from the Svix-verified webhook route.
...
Asynchronous tasks, audit log persistence, and post-creation setups are offloaded to an **in-process Go channel** (no external broker — Redis was removed; see the note in section 1) and processed by a pool of 5 worker goroutines started alongside the HTTP server in [cmd/api/main.go](backend/cmd/api/main.go), all running [worker.go](backend/internal/worker/worker.go)'s loop against the same shared `queue.Queue`.

```mermaid
sequenceDiagram
    autonumber
    participant Server as Go API Endpoint
    participant Chan as In-process Job Channel (queue.Queue)
    participant Worker as worker.go Loop (x5 goroutines)
    participant Provider as Cloudinary / PostgreSQL

    Server->>Chan: queue.PushJob(ctx, q, jobType, payload) — non-blocking send
    Note over Server,Chan: If the channel is full, PushJob fails immediately\nwith an error instead of blocking the request
    Chan-->>Worker: Job received off the channel
    Worker->>Worker: Unmarshal queue.Job.Type and Payload
    Worker->>Provider: Process Job (e.g. upload files, setup profiles)
    alt Job Succeeds
        Worker->>Worker: Log complete & wait for next job
    else Job Fails & attempts < MaxRetry
        Worker->>Worker: Wait delay (attempts * 5s)
        Worker->>Chan: Re-enqueue Job
    else Job Fails & attempts >= MaxRetry
        Worker->>Worker: Log and drop (no dead-letter queue)
    end
```
...
| `update_role_permission` | Sync role permissions | Applies a role's updated permission-string list to the `permissions` table. |
