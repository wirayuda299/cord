...
func CreateServer(ctx context.Context, container *databases.Container, srv *ServerPayload) *httputil.ErrorResponse {
	...
	rows, err := container.Postgres.Query(ctx, "insert into servers(name,created_by,banner_colors) values($1,$2,$3) returning id;", srv.Name, userID, []string{"#1f1f1f", "#3a3a3a"})
	if err != nil {
	...
	var memberID string
	if err := container.Postgres.QueryRow(ctx, "INSERT INTO members(server_id,user_id) VALUES($1,$2) RETURNING id", serverID, userID).Scan(&memberID); err != nil {
	...
	if err := queue.PushJob(ctx, container.Redis, queue.CreateDefaultServerSafety, &queue.CreateDefaultServerSafetyPayload{
		ServerID:  serverID,
		CreatedBy: userID,
	}); err != nil {
		return &httputil.ErrorResponse{
