...
func CreateDefaultServerProfile(ctx context.Context, db *databases.Container, p *queue.CreateDefaultServerProfilePayload) *httputil.ErrorResponse {
	...
	var u User
	err := db.Postgres.QueryRow(ctx, "SELECT username,avatar,avatar_asset_id, bio where id = $1", p.UserID).Scan(&u.Username, &u.Avatar, &u.AvatarID, &u.Bio)
	if err != nil {
	...
	_, err = db.Postgres.Exec(ctx, "INSERT INTO server_profile(server_id,user_id,username,avatar,avatar_asset_id,bio) values($1,$2,$3,$4,$5,$6)", p.ServerID, p.UserID, u.Username, u.Avatar, u.AvatarID, u.Bio)
	if err != nil {
		return &httputil.ErrorResponse{Err: err, Code: http.StatusInternalServerError}
	}
